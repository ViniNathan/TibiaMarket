...
class rashidMarket():
    def __init__(self):
        ...
        self.itemPrice = 0
        self.count = 0
        with open("scripts/rashid.json") as file:
            self.blueData = json.load(file)
            self.blueItens = self.blueData['itens']
    ...
    def priceFinder(self):
        ...
        save_path = "images/itemsPrice/rashid/{0}_price.png".format(self.nome_item)
        cv2.imwrite(save_path, itemPricePhoto)

    def priceImageToText(self):
        price_path = "images/itemsPrice/rashid/{0}_price.png".format(self.nome_item)
    ...
    def doBuy(self):
        ...
        if sliderMarketLocation is not None:
            ...
            if acceptButonPath is not None:
                ...
                print("A botão de compra foi encontrado na posição:", acceptButtonLocation)
    ...
    def run(self):
        for i in range(len(self.blueItens)):
            item = self.blueItens[i]
            self.nome_item = item['nome']
            self.valor_item = item['valor']
            self.marketSearchFinder()
            self.rashidSearchMarket()
            self.itemFinder()
            self.priceFinder()
